"""
Live Trading Bot using Alma Strategy (ALMA + RR TP/SL with ATR Swing Stop) with Roostoo API
...
- Entry via market order. Stop loss via ATR swing (volatility-buffered). Take profit via pre-placed limit order.
"""
...
from backtest_alma_pine import Params as AlmaParams, resample_ohlcv, variant, atr
...
class AlmaLiveTrader:
    def __init__(self, symbols: List[str], base_tf_min: int = 15, poll_seconds: int = 5,
        ...
        self._wallet_was_flat_last_sync = False
    ...
    def _sync_account_balance(self, balance_response: Optional[dict] = None) -> tuple[float, float]:
        """Sync with actual account balance from exchange"""
        try:
            if balance_response is None:
                balance_response = self.roostoo.get_balance()

            if not balance_response or not balance_response.get('Success'):
    ...
    def _sync_wallet_flat(self):
        if self._wallet_was_flat_last_sync:
            return
        try:
            balance_response = self.roostoo.get_balance()
            if balance_response and balance_response.get('Success'):
                ...
                for asset, balances in wallet.items():
                    ...
                    total_qty = float(balances.get('Free', 0)) + float(balances.get('Lock', 0))
                    if total_qty > 0:
                        logger.info(f"Closing existing {asset} position: {total_qty}")
                        self.roostoo.sell(asset, total_qty)
        except Exception as e:
            logger.error(f"Error syncing wallet flat: {e}")
        self._wallet_was_flat_last_sync = True
    ...
    def _load_params_for_symbol(self, symbol: str) -> AlmaParams:
        ...
        try:
            ...
            if os.path.exists(path):
                ...
                p.atr_len = int(bp.get('atr_len', 14))
                p.atr_mult = float(bp.get('atr_mult', 1.0))
                return p
        except Exception as e:
    ...
    def _compute_signal(self, symbol: str, base_df: pd.DataFrame, p: AlmaParams) -> Optional[dict]:
        # Need enough bars
        atr_len = getattr(p, 'atr_len', 14)
        if len(base_df) < max(50, p.basis_len + p.last_n_len + atr_len + 5):
            return None
        ...
        # ATR Swing Stop Loss: SL = lowest low of last_n_len bars - (ATR * atr_mult)
        lookback_slice = slice(max(0, i - p.last_n_len), i)
        window_low = float(base_df['low'].iloc[lookback_slice].min())
        
        # Calculate ATR on base timeframe
        atr_series = atr(base_df, atr_len)
        atr_val = float(atr_series.iloc[i - 1]) if i - 1 >= 0 and pd.notna(atr_series.iloc[i - 1]) else 0.0
        atr_mult = getattr(p, 'atr_mult', 1.0)
        
        # Apply ATR buffer to swing low
        sl = window_low - atr_mult * max(0.0, atr_val)
        
        entry = float(close_base.iloc[i])
        
        # Fallback if SL >= entry (invalid)
        if sl >= entry or not pd.notna(sl):
    ...
    def _place_tp_limit(self, symbol: str, qty: float, price: float, max_retries: int = 3) -> Optional[int]:
        ...
        step = QUANTITY_STEP_SIZES.get(symbol, 0.001)
        effective_qty = self._round_qty(symbol, min(qty, max(0.0, free)))

        if effective_qty <= 0:
            logger.warning(f"[{symbol}] Invalid TP quantity after balance check: requested={qty}, free={free}")
            return None

        attempt_qty = effective_qty
        for attempt in range(max_retries):
            try:
                resp = self.roostoo.sell(symbol, attempt_qty, price=price)
                if resp.get('Success'):
                    order_id = resp.get('OrderDetail', {}).get('OrderID')
                    logger.info(f"[{symbol}] TP limit order placed: ID={order_id}, qty={attempt_qty}, price={price}")
                    return order_id
                else:
                    err_msg = resp.get('ErrMsg', 'Unknown error')
                    logger.warning(f"[{symbol}] TP order failed (attempt {attempt+1}/{max_retries}): {err_msg}")
                    # On insufficient balance, reduce by one step and retry
                    if 'insufficient balance' in err_msg.lower():
                        reduced = self._round_qty(symbol, max(0.0, attempt_qty - step))
                        if reduced <= 0:
                            return None
                        attempt_qty = reduced
                        continue
        ...
        logger.error(f"[{symbol}] Failed to place TP order after {max_retries} attempts (last_qty={attempt_qty})")
        return None
    ...
    def _get_asset_free_locked(self, asset: str) -> tuple[float, float]:
        try:
            ...
            # Support both 'SpotWallet' and legacy 'Wallet' keys
    ...
    def _sync_cash_if_flat(self):
        """Refresh cash/equity from account when no positions remain in wallet."""
        if self.positions:
            self._wallet_was_flat_last_sync = False
            return
        ...
        try:
            ...
            for asset, info in wallet.items():
                if asset.upper() == 'USD':
                    continue
                free_amt = float((info or {}).get('Free', 0) or 0)
                locked_amt = float((info or {}).get('Lock', 0) or 0)
                if free_amt + locked_amt > 1e-8:
                    has_positions = True
                    break
            ...
            if not self._wallet_was_flat_last_sync:
                self.equity, self.cash = self._sync_account_balance(balance_response=balance_response)
                self._wallet_was_flat_last_sync = True
        except Exception as exc:
            logger.debug(f"Skipping flat cash sync: {exc}")
    ...
    def _check_tp_fill_and_close(self, symbol: str, pos: "Position") -> bool:
        ...
        try:
            ...
            finish_ts = int(od.get('FinishTimestamp') or 0)
            # Do NOT fall back to total Quantity when FilledQuantity is missing; that causes false positives
            filled_qty = float(od.get('FilledQuantity') or 0.0)
            avg_price = float(od.get('FilledAverPrice') or 0.0)

            # Strict fill detection per Roostoo API
            is_filled_status = status == 'filled'
            fully_filled_qty = filled_qty >= max(0.0, pos.quantity - 1e-12)
            has_valid_price = avg_price > 0
            has_finish_time = finish_ts > 0
            if is_filled_status and has_finish_time and fully_filled_qty and has_valid_price:
                ...
                if getattr(pos, 'trade_id', None):
                    self.supabase_logger.log_trade_exit(
                        ...
                        fees=fee,
                        is_dry_run=False,
                        metadata={'strategy': 'ALMA_ATR_SWING', 'tp_order_id': pos.tp_order_id}
                    )
    ...
    def run(self):
        ...
        # Register clear-on-exit handler
        if PORTFOLIO_CONFIG.get('clear_account_on_end', False):
            def _clear_on_exit():
                try:
            ...
            for symbol in self.symbols:
                ...
                if last_base_bar_time[symbol] is None or current_bar_time > last_base_bar_time[symbol]:
                    ...
                    if signal and symbol not in self.positions:
                        ...
                        # Determine actual available quantity after fill (cap by free balance)
                        asset = self._asset_from_symbol(symbol)
                        free, _locked = self._get_asset_free_locked(asset)
                        actual_qty = self._round_qty(symbol, min(qty, max(0.0, free)))

                        tp_id = self._place_tp_limit(symbol, actual_qty, signal['take_profit'])
                        pos = Position(symbol=symbol, entry_time=datetime.utcnow(), entry_price=actual_entry,
                                       quantity=actual_qty, stop_loss=signal['stop_loss'], take_profit=signal['take_profit'],
                                       order_id=order_id, tp_order_id=tp_id)
                        self.positions[symbol] = pos
                        # rough cash update
                        fee = actual_entry * actual_qty * (PORTFOLIO_CONFIG.get('fee_bps', 10.0) / 10000.0)
                        self.cash -= actual_entry * actual_qty + fee
                ...
                if symbol in self.positions:
                    ...
                    if last_price <= pos.stop_loss:
                        ...
                        if sell_qty <= 0:
                            # Dust handling: if the position itself is below tradeable thresholds, write it off locally
                            last_px = self._last_price(symbol)
                            step_qty = QUANTITY_STEP_SIZES.get(symbol, 0.001)
                            min_usd = RISK_CONFIG.get('min_position_value_usd', 1.0)
                            pos_value = (pos.quantity or 0.0) * (last_px or 0.0)
                            if pos.quantity < step_qty or pos_value < min_usd:
                                # Log exit as dust write-off without placing an order
                                fee = 0.0
                                pnl = (last_px - pos.entry_price) * pos.quantity if last_px > 0 else 0.0
                                pnl_pct = (pnl / (pos.entry_price * pos.quantity)) * 100 if pos.entry_price > 0 else 0.0
                                if getattr(pos, 'trade_id', None):
                                    self.supabase_logger.log_trade_exit(
                                        trade_id=pos.trade_id,
                                        ...
                                        realized_pnl=pnl,
                                        realized_pnl_pct=pnl_pct,
                                        exit_reason='DUST_WRITE_OFF',
                                        fees=fee,
                                        is_dry_run=False,
                                        metadata={'strategy': 'ALMA_ATR_SWING'}
                                    )
                                logger.info(f"[{symbol}] Position below tradeable threshold (qty={pos.quantity}, value=${pos_value:.2f}). Writing off as dust.")
                                del self.positions[symbol]
                                continue
...
if __name__ == '__main__':
    ...
    from live_alma_config import TRADING_PAIRS, TIMEFRAME_MINUTES
    ...
    trader = AlmaLiveTrader(symbols, base_tf_min=TIMEFRAME_MINUTES, poll_seconds=5)
    trader.run()
